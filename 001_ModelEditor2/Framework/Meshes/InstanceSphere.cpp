...
InstanceSphere::InstanceSphere(Shader * shader, float radius, UINT stackCount, UINT sliceCount)
	: InstanceMesh(shader), radius(radius), stackCount(stackCount), sliceCount(sliceCount)
{
...
void InstanceSphere::Create()
{
	...
	v.push_back(MeshVertex(0, -radius, 0, 0, 1, 0, -1, 0, 1, 0, 0));
