...
InstanceGrid::InstanceGrid(Shader * shader, float offsetU, float offsetV)
	: InstanceMesh(shader), offsetU(offsetU), offsetV(offsetV)
{
	texture = NULL;
}
...
void InstanceGrid::Create()
{
	UINT countX = 11;
	UINT countZ = 11; //정점의 개수
	...
	for (UINT z = 0; z < countZ; z++)
	{
		for (UINT x = 0; x < countX; x++)
		{
			...
			vertex.Normal = D3DXVECTOR3(0, 1, 0); //수직, 기울기가 없으르모
			vertex.Uv.x = (float)x / (float)(countX - 1) * offsetU;
			vertex.Uv.y = (float)z / (float)(countZ - 1) * offsetU;
