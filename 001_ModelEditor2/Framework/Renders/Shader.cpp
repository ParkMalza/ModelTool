...
Shader::Shader(wstring file)
	: file(L"../../_Shaders/" + file)  //내 프로젝트 두칸 위에 쉐이더 폴더를 의미
{
...
ID3D11InputLayout * Shader::CreateInputLayout(ID3DBlob * fxBlob, D3DX11_EFFECT_SHADER_DESC* effectVsDesc, vector<D3D11_SIGNATURE_PARAMETER_DESC>& params)
{
	...
	for (D3D11_SIGNATURE_PARAMETER_DESC& paramDesc : params)
	{
		...
		if (name == "POSITION")  
		{
			elementDesc.Format = DXGI_FORMAT_R32G32B32_FLOAT;  //shader에선 float4, 우린 float3로 다루겠다
			//elementDesc.InputSlot = paramDesc.SemanticIndex;
		}


		if (String::StartsWith(name, "INST") == true)  //인스턴스로 받아들임, INST로 시작하는거
		{
			elementDesc.InputSlot = 1;  //인스턴스 버퍼 슬롯 1번 쓰니까
			elementDesc.AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;
			elementDesc.InputSlotClass = D3D11_INPUT_PER_INSTANCE_DATA;  //인스턴스 데이타
			elementDesc.InstanceDataStepRate = 1;  // 인스턴스시 반드시 1이상 사용
		}

		if(String::StartsWith(name, "SV_") == false)  //레이아웃에 포함시키지 마라
...
void Shaders::GetEffect(wstring fileName, ID3DBlob** blob, ID3DX11Effect** effect)
{
	...
	if (shaders.count(fileName) < 1)
	{
		...
		//D3DCompileFromFile : HLSL 컴파일 한 후 메모리에 보관.
