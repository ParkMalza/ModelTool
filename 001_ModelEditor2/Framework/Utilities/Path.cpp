...
wstring Path::GetDirectoryFbxName(wstring path)
{
	String::Replace(&path, L"\\", L"/");
	size_t index = path.find_last_of('/');

	return path.substr(23, index + 1); // return L카추진/메쉬.fbx
}

wstring Path::GetDirectoryBeforeFbx(wstring path)
{
	String::Replace(&path, L"\\", L"/");
	size_t index = path.find_last_of('/');
	wstring temp = path.substr(23, path.size());
	wstring result = Path::GetDirectoryName(temp);
